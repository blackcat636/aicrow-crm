...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      onEscapeKeyDown={(e) => {
        // Ensure proper cleanup on ESC - call existing handler first
        if (props.onEscapeKeyDown) {
          props.onEscapeKeyDown(e)
        }
      }}
      onPointerDownOutside={(e) => {
        // Allow closing on outside click, but prevent if clicking on Select content
        const target = e.target as HTMLElement
        // Check if clicking on Select dropdown content
        if (target.closest('[data-radix-select-content]') || target.closest('[data-slot="select-content"]')) {
          e.preventDefault()
          return
        }
        // Call existing handler if provided
